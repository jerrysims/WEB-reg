class ClassRostersReport
  ...
  def create_body sheet, section
    ...
    sheet.row(0).concat %w{Student_first, Student_last, Pronouns WEB_Email Email Grade section_Name Day/Time Parent_Name Parent_Email Learning_Differences Race Address}
    sheet.row(0).default_format = Spreadsheet::Format.new weight: :bold

    row_index = 1
    section.students.each do |s|
      sheet.row(row_index).concat [
        s.first_name, s.last_name, s.pronouns, s.web_email, s.student_email, s.grade, section.name,
