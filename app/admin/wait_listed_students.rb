ActiveAdmin.register WaitListedStudent do
  form do |f|
    f.input :section, as: :select, collection: options_from_collection_for_select(Section.all.sort_by { |c| c.name }, :id, lambda { |c| "#{c.name}, #{c.day}, #{c.start_time.strftime("%l:%M")} (#{c.registration_period.name})" })
